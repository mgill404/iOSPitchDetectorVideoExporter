...
@interface SSEAudioFilePlayerII () {
...
@implementation SSEAudioFilePlayerII
@synthesize url = _url, loop=_loop, volume=_volume, pan=_pan, channelIsPlaying=_channelIsPlaying, channelIsMuted=_channelIsMuted, removeUponFinish=_removeUponFinish, completionBlock = _completionBlock, startLoopBlock = _startLoopBlock, audioDescription = _audioDescription;
...
+ (id)audioFilePlayerWithURL:(NSURL*)url audioController:(AEAudioController *)audioController error:(NSError **)error {
    ...
    SSEAudioFilePlayerII *player = [[self alloc] init];
    player->_volume = 1.0;
...
static void notifyLoopRestart(AEAudioController *audioController, void *userInfo, int length) {
    SSEAudioFilePlayerII *THIS = (__bridge SSEAudioFilePlayerII*)*(void**)userInfo;
...
static OSStatus renderCallback(__unsafe_unretained SSEAudioFilePlayerII *THIS, __unsafe_unretained AEAudioController *audioController, const AudioTimeStamp *time, UInt32 frames, AudioBufferList *audio) {
    ...
    if ( !THIS->_loop && playhead == THIS->_lengthInFrames ) {
        ...
        AEAudioControllerSendAsynchronousMessageToMainThread(audioController, notifyPlaybackStopped, &THIS, sizeof(SSEAudioFilePlayerII*));
        ...
        if ( playhead >= THIS->_lengthInFrames ) {
            ...
            if ( THIS->_loop ) {
                ...
                if ( THIS->_startLoopBlock ) {
                    ...
                    AEAudioControllerSendAsynchronousMessageToMainThread(audioController, notifyLoopRestart, &THIS, sizeof(SSEAudioFilePlayerII*));
                }
            } else {
                // Notify main thread that playback has finished
                AEAudioControllerSendAsynchronousMessageToMainThread(audioController, notifyPlaybackStopped, &THIS, sizeof(SSEAudioFilePlayerII*));
